...
// Mengambil action dari query string, defaultnya 'check_session'
$action = $_GET['action'] ?? 'check_session';

// Mengambil data JSON dari body request (untuk POST, PUT, DELETE)
$input = json_decode(file_get_contents('php://input'), true);
...
switch ($action) {
    case 'login':
        ...
        if ($stmt->fetchColumn() == 0) {
            ...
            $insertStmt->execute([$default_user, $default_pass_hashed, $default_role]);
        }
        ...
        if ($user && password_verify($password, $user['password'])) {
            unset($user['password']); // Hapus password dari data sesi
            $_SESSION['user'] = $user;
            json_response('success', $user);
        } else {
            json_response('error', ['message' => 'Username atau password salah.']);
        }
        break;
    ...
    default:
        if (!isset($_SESSION['user'])) {
            ...
            json_response('error', ['message' => 'Akses ditolak. Silakan login.']);
        }
        ...
        switch ($action) {
            ...
            case 'get_users':
                if ($currentUser['role'] !== 'Super Admin') json_response('error', ['message' => 'Akses ditolak.']);
            ...
            case 'add_user':
                ...
                try {
                    ...
                    json_response('success', ['id' => $pdo->lastInsertId(), 'username' => $input['username'], 'role' => $input['role']]);
                } catch (PDOException $e) {
                    json_response('error', ['message' => 'Username sudah ada.']);
                }
                break;
            ...
            case 'check_document':
                $doc_id = $input['id'] ?? '';
                $stmt = $pdo->prepare("SELECT doc_id as id, title FROM documents WHERE doc_id = ?");
                $stmt->execute([$doc_id]);
                $document = $stmt->fetch(PDO::FETCH_ASSOC);
            ...
            case 'get_documents':
                $stmt = $pdo->query("SELECT doc_id as id, title FROM documents ORDER BY created_at DESC");
                json_response('success', $stmt->fetchAll(PDO::FETCH_ASSOC));
                break;

            case 'add_document':
                if ($currentUser['role'] === 'Admin Invoice') json_response('error', ['message' => 'Akses ditolak.']);
            ...
            // Audit Log
            case 'get_audit_log':
                $stmt = $pdo->query("SELECT log.timestamp, log.doc_id, log.action, CONCAT(usr.username, ' (', usr.role, ')') as actor, log.details FROM audit_log log JOIN users usr ON log.user_id = usr.id ORDER BY log.timestamp DESC");
                json_response('success', $stmt->fetchAll(PDO::FETCH_ASSOC));
                break;
            
            case 'add_audit_log':
                $stmt = $pdo->prepare("INSERT INTO audit_log (doc_id, user_id, action, details) VALUES (?, ?, ?, ?)");
                $stmt->execute([$input['docId'], $currentUser['id'], $input['action'], $input['details']]);
                json_response('success');
                break;
        }
        break;
